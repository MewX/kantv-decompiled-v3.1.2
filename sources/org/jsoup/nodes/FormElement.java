...
public class FormElement extends Element {
    ...
    public Connection submit() {
        ...
        String absUrl = hasAttr(str) ? absUrl(str) : baseUri();
        Validate.notEmpty(absUrl, "Could not determine a form action URL for submit. Ensure you set a base URI when parsing.");
        return Jsoup.connect(absUrl).data((Collection<KeyVal>) formData()).method(attr(Param.METHOD).toUpperCase().equals("POST") ? Method.POST : Method.GET);
    }
            ...
            if (element.tag().isFormSubmittable() && !element.hasAttr("disabled")) {
                ...
                if (attr.length() != 0) {
                    ...
                    if ("select".equals(element.tagName())) {
                        ...
                        if (!z) {
                            Element first = element.select("option").first();
                            if (first != null) {
