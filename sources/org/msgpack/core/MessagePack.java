...
public class MessagePack {
    ...
    public static class PackerConfig implements Cloneable {
        ...
        public MessagePacker newPacker(OutputStream outputStream) {
            return newPacker((MessageBufferOutput) new OutputStreamBufferOutput(outputStream, this.bufferSize));
        }

        public MessagePacker newPacker(WritableByteChannel writableByteChannel) {
            return newPacker((MessageBufferOutput) new ChannelBufferOutput(writableByteChannel, this.bufferSize));
        }
    ...
    public static class UnpackerConfig implements Cloneable {
        ...
        public int hashCode() {
            int i = (((this.allowReadingStringAsBinary ? 1 : 0) * true) + (this.allowReadingBinaryAsString ? 1 : 0)) * 31;
            ...
            return ((((((hashCode + i2) * 31) + this.stringSizeLimit) * 31) + this.bufferSize) * 31) + this.stringDecoderBufferSize;
        }
        ...
        public MessageUnpacker newUnpacker(InputStream inputStream) {
            return newUnpacker((MessageBufferInput) new InputStreamBufferInput(inputStream, this.bufferSize));
        }

        public MessageUnpacker newUnpacker(ReadableByteChannel readableByteChannel) {
            return newUnpacker((MessageBufferInput) new ChannelBufferInput(readableByteChannel, this.bufferSize));
        }

        public MessageUnpacker newUnpacker(byte[] bArr) {
            return newUnpacker((MessageBufferInput) new ArrayBufferInput(bArr));
        }

        public MessageUnpacker newUnpacker(byte[] bArr, int i, int i2) {
            return newUnpacker((MessageBufferInput) new ArrayBufferInput(bArr, i, i2));
        }

        public MessageUnpacker newUnpacker(ByteBuffer byteBuffer) {
            return newUnpacker((MessageBufferInput) new ByteBufferInput(byteBuffer));
        }
