...
public final class Dispatcher {
    ...
    public void enqueue(AsyncCall asyncCall) {
        synchronized (this) {
            ...
            if (!asyncCall.get().forWebSocket) {
                AsyncCall findExistingCallWithHost = findExistingCallWithHost(asyncCall.host());
    ...
    @Nullable
    private AsyncCall findExistingCallWithHost(String str) {
        for (AsyncCall asyncCall : this.runningAsyncCalls) {
            if (asyncCall.host().equals(str)) {
                return asyncCall;
            }
        }
        for (AsyncCall asyncCall2 : this.readyAsyncCalls) {
