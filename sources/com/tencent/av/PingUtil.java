...
public class PingUtil implements Runnable {
            ...
            if (i5 <= allocate.remaining()) {
                ...
                for (int i6 = 0; i6 < b; i6++) {
                    ...
                    for (int i7 = 3; i7 >= 0; i7--) {
                        bArr3[i7] = allocate.get();
                    }
                    serverInfo.ip = InetAddress.getByAddress(bArr3);
                            ...
                            new Thread(new Runnable() {
                                public void run() {
                                    while (open.isOpen()) {
                                        try {
                                            if (open.select() != 0) {
                                                Iterator it = open.selectedKeys().iterator();
                                                while (it.hasNext()) {
                                                    if (((SelectionKey) it.next()).isReadable()) {
                                                        ...
                                                        allocate.get();
                                                        allocate.getShort();
                                                        Calendar calendar = (Calendar) PingUtil.this.timeRecord.get(Integer.valueOf(allocate.getInt()));
                                                        if (calendar != null) {
    ...
    public void logResult() {
        StringBuilder sb = new StringBuilder();
        sb.append("ping result from ");
        sb.append(this.pingResults.size());
        sb.append(" servers");
        ...
        for (PingResult pingResult : this.pingResults) {
            ...
            sb3.append("serverIp:");
            sb3.append(pingResult.server.ip.toString());
            sb3.append(" useTime:");
            sb3.append(pingResult.useTime);
            sb3.append(" totalPkg:");
            sb3.append(pingResult.totalPkg);
            sb3.append(" receivePkg:");
            sb3.append(pingResult.receivePkg);
    ...
    private void reportToServer() {
        ...
        allocate.clear();
        allocate.put(2).put(1).putShort(7).putShort(6).putInt(sdkAppId).putInt(23678484).putInt(i).put(size);
        for (PingResult pingResult : this.pingResults) {
        ...
        allocate.putShort(0);
        allocate.putShort(0);
        TIMIntManager.getInstance().requestMultiVideoInfo(NetworkUtil.formReq(this.user.getTinyId(), AVException.USERNAME_PASSWORD_MISMATCH, 0, "", allocate.array()), new TIMValueCallBack<byte[]>() {
            public void onError(int i, String str) {
                StringBuilder sb = new StringBuilder();
                sb.append("reportError: code ");
                sb.append(i);
                sb.append(" desc ");
                sb.append(str);
                QLog.e(PingUtil.tag, sb.toString());
            }

            public void onSuccess(byte[] bArr) {
                QLog.e(PingUtil.tag, "ping report Success");
            }
        });
        this.running = false;
    }
