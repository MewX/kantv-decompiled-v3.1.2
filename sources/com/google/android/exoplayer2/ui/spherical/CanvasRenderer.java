...
public final class CanvasRenderer {
    ...
    public void init() {
        if (this.program == 0) {
            ...
            this.displaySurfaceTexture.setOnFrameAvailableListener(new OnFrameAvailableListener() {
    ...
    static PointF internalTranslateClick(float f, float f2, float f3, float f4, float f5, float f6, int i, int i2) {
        if (f < HALF_PI && f > -1.5707964f && f2 < HALF_PI && f2 > -1.5707964f) {
            double tan = Math.tan((double) f) * 1.0d;
            double d = (double) f3;
            Double.isNaN(d);
            double d2 = tan - d;
            double tan2 = Math.tan((double) f2) * 1.0d;
            double d3 = (double) f4;
            Double.isNaN(d3);
            double d4 = tan2 - d3;
            if (d2 >= 0.0d) {
                ...
                if (d2 <= d5 && d4 >= 0.0d) {
                    ...
                    if (d4 <= d6) {
                        ...
                        double d12 = d11 / d6;
                        Double.isNaN(d10);
                        return new PointF(f7, (float) (d10 - d12));
                    }
                }
            }
        }
