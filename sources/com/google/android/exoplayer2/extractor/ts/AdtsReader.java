...
public final class AdtsReader implements ElementaryStreamReader {
    ...
    private void findNextSample(ParsableByteArray parsableByteArray) {
        ...
        while (position < limit) {
            int i = position + 1;
            byte b = bArr[position] & Draft_75.END_OF_FRAME;
            if (this.matchState != 512 || !isAdtsSyncBytes(-1, (byte) b) || (!this.foundFirstFrame && !checkSyncPositionValid(parsableByteArray, i - 2))) {
    ...
    private boolean checkSyncPositionValid(ParsableByteArray parsableByteArray, int i) {
        ...
        if (!isAdtsSyncBytes(parsableByteArray.data[i3], parsableByteArray.data[i4]) || !(this.firstFrameVersion == -1 || ((parsableByteArray.data[i4] & 8) >> 3) == readBits)) {
            z = false;
        }
        return z;
    }
    ...
    private boolean isAdtsSyncBytes(byte b, byte b2) {
        return isAdtsSyncWord(((b & Draft_75.END_OF_FRAME) << 8) | (b2 & Draft_75.END_OF_FRAME));
    }
