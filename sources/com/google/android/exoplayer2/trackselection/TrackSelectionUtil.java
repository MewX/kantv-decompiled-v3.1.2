...
public final class TrackSelectionUtil {
    ...
    public static Parameters updateParametersWithOverride(Parameters parameters, int i, TrackGroupArray trackGroupArray, boolean z, @Nullable SelectionOverride selectionOverride) {
    ...
    static int[] getBitratesUsingPastInfo(List<? extends MediaChunk> list, Format[] formatArr, long j, @Nullable int[] iArr) {
        ...
        int averageQueueBitrate = getAverageQueueBitrate(list, j);
        if (averageQueueBitrate == -1) {
            return iArr;
        }
    ...
    private static void estimateBitrates(int[] iArr, Format[] formatArr, int[] iArr2, float[] fArr) {
        for (int i = 0; i < iArr.length; i++) {
            if (iArr[i] == -1) {
                int i2 = formatArr[i].bitrate;
                if (i2 != -1) {
                    iArr[i] = (int) (fArr[getClosestBitrateIndex(i2, iArr2)] * ((float) i2));
                }
            }
        }
    }
