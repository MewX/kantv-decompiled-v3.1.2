...
public class Jdk8DateCodec extends ContextObjectDeserializer implements ObjectSerializer, ContextObjectSerializer, ObjectDeserializer {
    ...
    static {
        String str = defaultPatttern;
        defaultFormatter = DateTimeFormatter.ofPattern(str);
        ISO_FIXED_FORMAT = DateTimeFormatter.ofPattern(str).withZone(ZoneId.systemDefault());
    }
    ...
    public <T> T deserialze(DefaultJSONParser defaultJSONParser, Type type, Object obj, String str, int i) {
        ...
        if (jSONLexer.token() == 8) {
            ...
            jSONLexer.nextToken();
            DateTimeFormatter dateTimeFormatter = str != null ? defaultPatttern.equals(str) ? defaultFormatter : DateTimeFormatter.ofPattern(str) : null;
            if ("".equals(stringVal)) {
                return null;
            }
            if (type == LocalDateTime.class) {
                if (stringVal.length() == 10 || stringVal.length() == 8) {
                    t3 = LocalDateTime.of(parseLocalDate(stringVal, str, dateTimeFormatter), LocalTime.MIN);
                } else {
                    t3 = parseDateTime(stringVal, dateTimeFormatter);
                }
                return t3;
                ...
                if (stringVal.length() == 23) {
                    ...
                    t2 = parseLocalDate(stringVal, str, dateTimeFormatter);
                }
                return t2;
